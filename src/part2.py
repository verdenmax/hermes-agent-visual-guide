...
<p><span class="mono">check_fn</span> 门控之所以优雅，在于它把「按需付费」做成了<strong>默认安全</strong>。看 <span class="mono">_HERMES_CORE_TOOLS</span> 里那几组带门的工具就懂了：Home Assistant 的 <span class="mono">ha_*</span> 只在配了 <span class="mono">HASS_TOKEN</span> 时出现，<span class="mono">computer_use</span> 要装了 cua-driver 才上台，<span class="mono">kanban_*</span> 要么是被派成 kanban worker（<span class="mono">HERMES_KANBAN_TASK</span> 环境变量在）、要么是 profile 显式启用了 kanban toolset 才进 schema（注释见 <span class="mono">toolsets.py:31</span>）。这意味着：这些工具名虽然<strong>常驻</strong>在核心清单里，却<strong>不向没配前置条件的用户收一分 token 税</strong>——它们的成本只在「你真的用得上」时才发生。这就是「能力在边缘」的另一种写法：把昂贵的核心面，按每个用户的真实环境<strong>动态裁掉</strong>。换句话说，同一份核心清单，在没配智能家居的用户那里和配了的用户那里，发给模型的<strong>实际 schema 并不一样</strong>——门控让「窄」具体到了每个会话的环境指纹。</p>
...
<p>再往深一层看那条「handler <strong>必返回 JSON 字符串</strong>」的硬规矩（AGENTS.md：「All handlers MUST return a JSON string」）。它不是风格洁癖，而是<strong>统一契约</strong>：成功结果、报错、空值，全部以同一种「JSON 字符串 → tool 消息」的形状回到对话里，<span class="mono">dispatch</span> 才能对<strong>所有</strong>工具一视同仁地包裹异常、净化噪声，而不必为每个工具特判返回类型。配套的还有 <span class="mono">coerce_tool_args</span>（<span class="mono">model_tools.py:643</span>）——开源模型常把整数 <span class="mono">42</span> 写成字符串 <span class="st">"42"</span>、把单个 URL 漏写成数组，它会按 schema 声明的类型把参数<strong>悄悄纠回去</strong>，而不是直接抛一个让模型困惑的工具失败。统一的入参强制 + 统一的出参契约，一头一尾把脆弱的结构化调用（约束 E）夹稳。</p>
...
<p>Look one level deeper at the hard rule "handlers <strong>must return a JSON string</strong>" (AGENTS.md: "All handlers MUST return a JSON string"). It isn't stylistic fussiness — it's a <strong>uniform contract</strong>: success results, errors, and empty values all return to the conversation in the same "JSON string → tool message" shape, which is exactly what lets <span class="mono">dispatch</span> wrap exceptions and sanitize noise for <strong>every</strong> tool uniformly, without special-casing return types per tool. Its counterpart is <span class="mono">coerce_tool_args</span> (<span class="mono">model_tools.py:643</span>) — open-weight models often emit the integer <span class="mono">42</span> as the string <span class="st">"42"</span>, or a single URL where an array is expected, and it <strong>quietly fixes</strong> the args back to the schema-declared types instead of throwing a confusing tool failure. Uniform input coercion + a uniform output contract brace the brittle structured call (constraint E) at both ends.</p>
