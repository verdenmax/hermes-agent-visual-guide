...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>Cache intact</strong>: control commands ride a control channel, never mixing into conversation history, preserving strict role alternation (ch.7).</li>
...
<p>The auth choice here is instructive too: <span class="mono">/api/pty</span>'s WebSocket carries the token as a <span class="mono">?token=&lt;session&gt;</span> query param rather than an <span class="mono">Authorization</span> header — because browsers <strong>can't set custom headers</strong> on a WS upgrade handshake, leaving the URL as the only channel. The token is the very same ephemeral <span class="mono">_SESSION_TOKEN</span> as REST, and the whole channel is <strong>loopback-only by default</strong>, rejecting non-loopback clients unless auth is explicitly enabled or it's bound to <span class="mono">0.0.0.0</span>, locking the "real PTY exposed to a browser" risk down to the local machine. The server also serializes chat-argv resolution with a <span class="mono">chat_argv_lock</span> so concurrent <span class="mono">/api/pty</span> connections don't race over the same launch args. All of it makes "project a CLI process into a web page" both usable and tight.</p>
...
<svg viewBox="0 0 680 348" role="img" aria-label="config.yaml holds behavioral settings, .env holds credentials, and .env still mixes in a few non-secret entries">
  ...
  <text x="374" y="255" font-size="10" fill="var(--ink)">quite a few (66) password:False entries:</text>
  <text x="374" y="273" font-size="10" fill="var(--muted)" font-family="monospace">NOUS_BASE_URL / GEMINI_BASE_URL</text>
