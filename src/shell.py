"""Shared HTML shell (CSS design system + navigation) for the Hermes Agent visual guide."""
...
_FAVICON_SVG = (
    ...
    "<rect width='32' height='32' rx='7' fill='#c8a02c'/>"
...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --accent: #e3c05a; --accent-soft: #2e2510; --accent-ink: #f0d488;
...
/* 🖼️ standalone SVG figure (hand-drawn diagrams, colors via CSS vars for light/dark) */
.figure { margin: 1.4rem 0; background: var(--panel); border: 1px solid var(--line);
  border-radius: var(--radius); padding: 1rem 1.1rem .7rem; box-shadow: var(--shadow); }
.figure svg { width: 100%; height: auto; display: block; font-family: var(--ui, system-ui, -apple-system, "Segoe UI", Roboto, sans-serif); }
.figure .fig-cap { font-size: .8rem; color: var(--muted); margin-top: .55rem; text-align: center; line-height: 1.5; }
.figure .fig-cap b { color: var(--accent-ink); font-weight: 700; }
.figure .svg-t { fill: var(--ink); }
.figure .svg-m { fill: var(--muted); }
.figure .svg-f { fill: var(--faint); }
.figure .svg-line { stroke: var(--line); }
.figure .svg-acc { fill: var(--accent); }
.figure .svg-accs { fill: var(--accent-soft); }
.figure .svg-blue { fill: var(--blue); }
.figure .svg-blues { fill: var(--blue-soft); }
.figure .svg-panel { fill: var(--panel); }
.figure .svg-panel2 { fill: var(--panel-2); }
...
.badge.constraint { display:inline-block; font-size:.7rem; font-weight:700;
  ...
  color:var(--purple); border:1px solid var(--purple); margin:0 .15rem; white-space:nowrap; }
"""
...
LANG_JS = """
function havgSetLang(l){
  l=(l==='en')?'en':'zh';
  var d=document.documentElement;
  d.dataset.lang=l; d.lang=(l==='en'?'en':'zh-CN');
  try{localStorage.setItem('havg-lang',l);}catch(e){}
}
function havgToggleLang(){
  havgSetLang(document.documentElement.dataset.lang==='en'?'zh':'en');
}
"""

# Runs in <head> before first paint to avoid a flash of the wrong language.
LANG_BOOT = (
    "<script>try{var l=localStorage.getItem('havg-lang');"
...
def page(filename, content, home_href="../index.html"):
    ...
    title_tag = f"{idx+1:02d} · {title_zh} / {title_en} · Hermes Agent 设计图解"
    desc = f"{part_zh}｜{title_zh} · Hermes Agent 设计图解（中英双语，配真实源码与 prompt 原文、协作机制与设计取舍）"
...
<div class="topbar">
  <div class="topbar-inner">
    <a class="home" href="{home}">☤ <b class="lang-zh">Hermes 设计图解</b><b class="lang-en">Hermes Agent Design Guide</b></a>
    <span class="pill">{bi(esc(part_zh), esc(part_en))}</span>
    <span class="pill">{idx+1:02d} / {total:02d}</span>
    <button class="langtoggle" onclick="havgToggleLang()" aria-label="switch language"><span class="lang-zh">EN</span><span class="lang-en">中</span></button>
  </div>
...
SUBTITLES = {
    "01-what-is-hermes.html": ("自我进化的个人 AI agent · 学习闭环 · 处处可达",
                               "A self-improving personal AI agent; learning loop; lives everywhere"),
    "02-llm-constraints-single-call.html": ("单次调用的约束 · 注意力 U 型 · 无状态纯函数 · 分词之痛",
                               "Single-call limits; U-shaped attention; stateless pure function; tokenization"),
    "03-llm-constraints-autonomy.html": ("自主性放大约束 · 幻觉 · 指令=数据 · 误差累积",
                               "Autonomy amplifies the limits; hallucination; instructions=data; error compounding"),
    "04-project-map-narrow-waist.html": ("一个核心驱动多端 · 窄腰 _HERMES_CORE_TOOLS · 能力在边缘",
                               "One core, many edges; the narrow waist; capability at the edges"),
    "05-conversation-lifecycle.html": ("同步 while 主循环 · 可中断 · 迭代预算",
                               "The synchronous while loop; interruptible; iteration budget"),
    "06-system-prompt-caching.html": ("三层前缀逐字节稳定 · 4 个 cache_control 断点 · 省约 75%",
                               "Three-tier byte-stable prefix; 4 cache_control breakpoints; ~75% cheaper"),
    "07-message-flow-providers.html": ("统一 messages · 四 transport 翻译 · 严格角色交替",
                               "Unified messages; four-transport translation; strict role alternation"),
    "08-tool-system.html": ("注册·门控·分派·预算 · check_fn 零足迹 · Footprint Ladder",
                               "Register, gate, dispatch, budget; check_fn zero-footprint; the Footprint Ladder"),
    "09-learning-nudge-skills.html": ("每约 10 轮后台 review · 不破缓存 · 技能 append 为 user 消息",
                               "Background review every ~10 turns; cache intact; skills append as a user message"),
    "10-curator.html": ("时间戳状态机 · active→stale→archived · 永不删只归档",
                               "Timestamp state machine; active→stale→archived; never deletes, only archives"),
    "11-memory.html": ("外置磁盘 · 会话开始冻结快照 · 写入不破缓存",
                               "Externalized to disk; frozen snapshot at session start; writes keep the cache"),
    "12-session-search.html": ("倒排索引 + BM25 · 零 LLM 召回片段 · CJK trigram 兜底",
                               "Inverted index + BM25; zero-LLM snippet recall; CJK trigram fallback"),
    "13-delegation.html": ("隔离上下文 · 只回一句摘要 · leaf 最小权限",
                               "Isolated context; one-line summary back; leaf least-privilege"),
    "14-review-verification.html": ("没有 agent 该验证自己 · 验证者只拿 diff · fail-closed",
                               "No agent verifies itself; reviewer gets only the diff; fail-closed"),
    "15-context-compression.html": ("50% 阈值 + 防抖 · 保头·摘要中段·保尾 · 缓存唯一例外",
                               "50% threshold + debounce; keep head, summarize middle, keep tail; the only cache exception"),
    "16-terminal-backends.html": ("一个 terminal 工具 · BaseEnvironment 统一 · 六后端 local/docker/ssh…",
                               "One terminal tool; the BaseEnvironment contract; six backends local/docker/ssh…"),
    "17-gateway-adapters.html": ("归一化 MessageEvent · 28+ 平台边缘适配 · 差异沉到边缘",
                               "Normalized MessageEvent; 28+ platform edge adapters; differences sink to the edge"),
    "18-gateway-guards.html": ("两道顺序守卫 · 控制命令必须旁路 · 不让模型当裁判",
                               "Two sequential guards; control commands must bypass; the model is not the judge"),
    "19-tui-desktop.html": ("两进程 Node + Python · JSON-RPC over stdio · PTY 投进浏览器",
                               "Two processes Node + Python; JSON-RPC over stdio; PTY into the browser"),
    "20-config-profiles.html": ("HERMES_HOME 抢先隔离 · config.yaml vs .env · 深合并",
                               "HERMES_HOME isolates first; config.yaml vs .env; deep-merge"),
    "21-cron-kanban.html": ("调度 tick + grace 窗口 · 独立会话不镜像 · 多 worker 看板",
                               "Scheduler tick + grace window; isolated non-mirrored sessions; multi-worker board"),
    "22-eval-batch-trajectory.html": ("并行 Pool · ShareGPT 轨迹喂训练 · 锁行为不锁快照",
                               "Parallel Pool; ShareGPT trajectories for training; assert behavior, not snapshots"),
    "23-plugins-skills-mcp.html": ("三条边缘扩展 · 都不改核心 · MCP 进 catalog",
                               "Three edge extensions; none touch the core; MCP joins the catalog"),
    "24-security.html": ("纵深防御多层 · HARDLINE 连 yolo 都拦 · 确定性代码把关",
                               "Defense in depth; HARDLINE blocks even yolo; deterministic code guards"),
    "25-design-principles.html": ("三条设计线 · A–G 约束矩阵 · 一个概率内核的敌人",
                               "Three design lines; the A–G constraint matrix; one probabilistic enemy"),
    "26-pitfalls-building-an-agent.html": ("真实坑：症状→根因→对策 · 避坑速查",
                               "Real pitfalls: symptom → cause → fix; a quick-reference playbook"),
    "27-model-credentials-routing.html": ("凭证池状态机 · 副 LLM 路由 · 上下文窗口解析",
                               "Credential-pool state machine; side-LLM routing; context-window resolution"),
    "28-runtime-resilience.html": ("协作式中断扇出 · 后台进程去重 · 原子文件编辑",
                               "Cooperative interrupt fan-out; background-process dedup; atomic file edits"),
}
...
def index_page(lesson_prefix="lessons/"):
    ...
    title_tag = "Hermes Agent 设计图解 · 理解每个设计取舍 / Hermes Agent Design Guide"
    desc = ("理解 hermes-agent 设计取舍的中英双语图解指南：宏观架构、agent 核心、自我进化闭环、"
            "多端网关、自动化与研究，每章配真实源码与 prompt 原文、协作机制与设计取舍。")
...
<div class="topbar">
  <div class="topbar-inner">
    <span class="home">☤ <b class="lang-zh">Hermes 设计图解</b><b class="lang-en">Hermes Agent Design Guide</b></span>
...
<div class="wrap">
  <div class="hero index">
    <div class="part">{bi("理解设计 · 面向工程师与面试", "Design-first · for engineers & interviews")}</div>
    <h1><span class="lang-zh">用图解理解 hermes-agent 的每个设计</span><span class="lang-en">Understand every hermes-agent design decision, visually</span></h1>
    <p class="lead"><span class="lang-zh">这套指南带你<strong>沿着设计线</strong>理解 hermes-agent：先认清 <strong>LLM 的固有约束</strong>，再逐个看每个部件<strong>为对抗哪个约束而设计</strong>、它如何与全局<strong>协作</strong>。每章配<strong>真实源码与 prompt 原文</strong>、手绘图、🧩协作机制框与🎯设计取舍框。</span><span class="lang-en">A design-first tour of hermes-agent: first see the <strong>inherent LLM constraints</strong>, then read each part as an <strong>engineering answer</strong> to one of them, and how it <strong>cooperates</strong> with the whole. Every chapter ships <strong>real source &amp; prompt excerpts</strong>, hand-drawn diagrams, a 🧩 collaboration box and a 🎯 design-tradeoff box.</span></p>
    <div class="legend">
    ...
    <p style="margin:.8rem 0 0;color:var(--faint);font-size:.8rem">{bi("📌 对照 hermes-agent 仓库真实源码核实 · 引用以「文件:行号」标注", "📌 Verified against the real hermes-agent source; references cite file:line")}</p>
  </div>
