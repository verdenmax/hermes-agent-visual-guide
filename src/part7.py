...
<div class="figure">
<svg viewBox="0 0 680 418" role="img" aria-label="纵深防御四层：一个威胁要依次穿过四道独立的关卡才能触及真实权限">
  <text x="340" y="22" text-anchor="middle" font-size="13.5" font-weight="700" fill="var(--red)">▼ 一个威胁要穿过 4 道独立关卡</text>
  <rect x="172" y="34" width="336" height="44" rx="9" fill="var(--red-soft)" stroke="var(--red)" stroke-width="2"/>
  <text x="340" y="61" text-anchor="middle" font-size="12" fill="var(--red)">恶意输入 · rm -rf / · 注入指令 · 中毒技能</text>
  <text x="340" y="93" text-anchor="middle" font-size="13" fill="var(--faint)">▾</text>
  <rect x="40" y="98" width="600" height="52" rx="9" fill="var(--accent-soft)" stroke="var(--accent)"/>
  <circle cx="74" cy="124" r="15" fill="var(--panel)" stroke="var(--accent)" stroke-width="1.5"/>
  <text x="74" y="129" text-anchor="middle" font-size="13" font-weight="700" fill="var(--accent-ink)">1</text>
  <text x="100" y="120" font-size="13" font-weight="700" fill="var(--ink)">危险命令审批</text>
  <text x="100" y="139" font-size="10.5" fill="var(--muted)">detect_dangerous_command · 正则黑名单 · HARDLINE 连 /yolo 都拦</text>
  <text x="340" y="156" text-anchor="middle" font-size="13" fill="var(--faint)">▾</text>
  <rect x="40" y="158" width="600" height="52" rx="9" fill="var(--accent-soft)" stroke="var(--accent)"/>
  <circle cx="74" cy="184" r="15" fill="var(--panel)" stroke="var(--accent)" stroke-width="1.5"/>
  <text x="74" y="189" text-anchor="middle" font-size="13" font-weight="700" fill="var(--accent-ink)">2</text>
  <text x="100" y="180" font-size="13" font-weight="700" fill="var(--ink)">子代理最小权限</text>
  <text x="100" y="199" font-size="10.5" fill="var(--muted)">剥离 DELEGATE_BLOCKED_TOOLS · 只给够用的工具</text>
  <text x="340" y="216" text-anchor="middle" font-size="13" fill="var(--faint)">▾</text>
  <rect x="40" y="218" width="600" height="52" rx="9" fill="var(--accent-soft)" stroke="var(--accent)"/>
  <circle cx="74" cy="244" r="15" fill="var(--panel)" stroke="var(--accent)" stroke-width="1.5"/>
  <text x="74" y="249" text-anchor="middle" font-size="13" font-weight="700" fill="var(--accent-ink)">3</text>
  <text x="100" y="240" font-size="13" font-weight="700" fill="var(--ink)">注入隔离</text>
  <text x="100" y="259" font-size="10.5" fill="var(--muted)">网关两道守卫 · 控制命令从数据流分离（第 18 章）</text>
  <text x="340" y="276" text-anchor="middle" font-size="13" fill="var(--faint)">▾</text>
  <rect x="40" y="278" width="600" height="52" rx="9" fill="var(--accent-soft)" stroke="var(--accent)"/>
  <circle cx="74" cy="304" r="15" fill="var(--panel)" stroke="var(--accent)" stroke-width="1.5"/>
  <text x="74" y="309" text-anchor="middle" font-size="13" font-weight="700" fill="var(--accent-ink)">4</text>
  <text x="100" y="300" font-size="13" font-weight="700" fill="var(--ink)">供应链锁定</text>
  <text x="100" y="319" font-size="10.5" fill="var(--muted)">依赖全 pin（== / commit SHA）· 不信任会自动变的东西</text>
  <text x="340" y="338" text-anchor="middle" font-size="13" fill="var(--accent)">▾</text>
  <rect x="172" y="342" width="336" height="44" rx="9" fill="var(--panel-2)" stroke="var(--line)"/>
  <text x="340" y="369" text-anchor="middle" font-size="12" font-weight="700" fill="var(--ink)">🔒 系统 · 真实权限（终端 / 文件 / 浏览器 / 消息）</text>
  <text x="340" y="408" text-anchor="middle" font-size="12" font-weight="700" fill="var(--muted)">单层失守，下一层兜底 · 绝不靠模型自己判断</text>
</svg>
<div class="fig-cap"><b>纵深防御四层</b>：一个恶意输入要触及 Hermes 的真实权限，必须<b>依次</b>穿过四道<b>互相独立</b>的关卡——① 危险命令审批（<span class="mono">detect_dangerous_command</span> 正则黑名单，HARDLINE 连 <span class="mono">/yolo</span> 都拦）→ ② 子代理最小权限（剥离 <span class="mono">DELEGATE_BLOCKED_TOOLS</span>）→ ③ 注入隔离（网关两道守卫，第 18 章）→ ④ 供应链锁定（依赖全 pin）。每一层都用<b>确定性代码</b>把关：<b>单层失守，下一层兜底；绝不靠模型自己判断</b>。</div>
</div>
...
<div class="figure">
<svg viewBox="0 0 680 300" role="img" aria-label="HARDLINE 与 DANGEROUS 两类危险命令按不可逆程度分级">
  <text x="340" y="22" text-anchor="middle" font-size="13.5" font-weight="700" fill="var(--ink)">两类危险命令 · 按不可逆程度分级上锁</text>
  <rect x="28" y="38" width="300" height="200" rx="10" fill="var(--red-soft)" stroke="var(--red)"/>
  <rect x="28" y="38" width="300" height="34" rx="10" fill="var(--red)"/>
  <text x="178" y="60" text-anchor="middle" font-size="12.5" font-weight="700" fill="var(--panel)">HARDLINE · 12 条 · 红线</text>
  <g font-size="11" font-family="monospace" fill="var(--ink)">
    <text x="44" y="98">rm -rf /  ·  /home /etc /usr</text>
    <text x="44" y="122">mkfs（格式化文件系统）</text>
    <text x="44" y="146">dd of=/dev/sd*（写裸盘）</text>
    <text x="44" y="170">:(){ :|:&amp;};:（fork 炸弹）</text>
  </g>
  <rect x="44" y="186" width="234" height="26" rx="13" fill="var(--red)"/>
  <text x="161" y="204" text-anchor="middle" font-size="11" font-weight="700" fill="var(--panel)">⛔ 连 /yolo 都拦 · 零容忍</text>
  <text x="178" y="230" text-anchor="middle" font-size="10.5" fill="var(--red)">不可逆 · 几乎无正当理由 · 代价无穷大</text>
  <rect x="352" y="38" width="300" height="200" rx="10" fill="var(--amber-soft)" stroke="var(--amber)"/>
  <rect x="352" y="38" width="300" height="34" rx="10" fill="var(--amber)"/>
  <text x="502" y="60" text-anchor="middle" font-size="12.5" font-weight="700" fill="var(--panel)">DANGEROUS · 61 条 · 黄线</text>
  <g font-size="11.5" fill="var(--ink)">
    <text x="368" y="98">命中即需用户审批后执行</text>
    <text x="368" y="122">/yolo 可一次放行</text>
    <text x="368" y="146">智能审批自动放行低风险</text>
    <text x="368" y="170">该层仅放宽 · 无权松动红线</text>
  </g>
  <rect x="368" y="186" width="234" height="26" rx="13" fill="var(--amber)"/>
  <text x="485" y="204" text-anchor="middle" font-size="11" font-weight="700" fill="var(--panel)">⚠ /yolo 可放行 · 省心口子</text>
  <text x="502" y="230" text-anchor="middle" font-size="10.5" fill="var(--amber)">可逆 · 受控场景 · 有经验用户</text>
  <text x="70" y="270" font-size="11" font-weight="700" fill="var(--red)">◀ 不可逆（焊死）</text>
  <text x="340" y="270" text-anchor="middle" font-size="11" fill="var(--muted)">按不可逆程度分级</text>
  <text x="610" y="270" text-anchor="end" font-size="11" font-weight="700" fill="var(--amber)">可逆（给方便）▶</text>
</svg>
...
<div class="figure">
<svg viewBox="0 0 680 524" role="img" aria-label="一条混淆命令 r反斜杠m -rf /home/alice/* 被第一道防线归一化还原成 rm -rf ~/* 后命中 HARDLINE 正则被硬阻断的真实数据流：① 攻击输入插入转义反斜杠和空引号躲正则；② _normalize_command_for_detection 多步归一化（strip_ansi 去 ANSI、去 null 字节、NFKC 规范化、re.sub 剥反斜杠 r反斜杠m 变 rm、剥空引号、_rewrite_resolved_user_home 把 /home/alice/ 变 ~/）还原成 rm -rf ~/*；③ 命中 HARDLINE 正则 recursive delete of home directory；④ _hardline_block_result 硬阻断 BLOCKED hardline，连 --yolo /yolo approvals.mode=off 都不放行；⑤ 对照：若改问模型危不危险，模型可能答这是安全清理脚本就放行，正则确定性命中而措辞混淆无效。">
  ...
  <text x="58" y="194" font-size="9" fill="var(--muted)">3  unicodedata.normalize('NFKC')</text>
  <text x="58" y="212" font-size="9" fill="var(--purple)">4  re.sub(r'\\([^\n])', r'\1')</text>
  <text x="232" y="212" font-size="9" fill="var(--muted)">剥反斜杠 r\m→rm · approval.py:575</text>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>危险命令审批</strong>:<span class="mono">detect_dangerous_command</span> 用正则黑名单(12 HARDLINE + 61 DANGEROUS);HARDLINE 红线连 <span class="mono">/yolo</span> 都拦。</li>
...
<div class="figure">
<svg viewBox="0 0 680 418" role="img" aria-label="Defense in depth: a threat must pass four independent gates in series before it reaches real privileges">
  <text x="340" y="22" text-anchor="middle" font-size="13.5" font-weight="700" fill="var(--red)">▼ one threat must pass 4 independent gates</text>
  <rect x="158" y="34" width="364" height="44" rx="9" fill="var(--red-soft)" stroke="var(--red)" stroke-width="2"/>
  <text x="340" y="61" text-anchor="middle" font-size="12" fill="var(--red)">malicious input · rm -rf / · injected instructions · poisoned skill</text>
  <text x="340" y="93" text-anchor="middle" font-size="13" fill="var(--faint)">▾</text>
  <rect x="40" y="98" width="600" height="52" rx="9" fill="var(--accent-soft)" stroke="var(--accent)"/>
  <circle cx="74" cy="124" r="15" fill="var(--panel)" stroke="var(--accent)" stroke-width="1.5"/>
  <text x="74" y="129" text-anchor="middle" font-size="13" font-weight="700" fill="var(--accent-ink)">1</text>
  <text x="100" y="120" font-size="13" font-weight="700" fill="var(--ink)">Dangerous-command approval</text>
  <text x="100" y="139" font-size="10.5" fill="var(--muted)">detect_dangerous_command · regex blacklist · HARDLINE not even /yolo clears</text>
  <text x="340" y="156" text-anchor="middle" font-size="13" fill="var(--faint)">▾</text>
  <rect x="40" y="158" width="600" height="52" rx="9" fill="var(--accent-soft)" stroke="var(--accent)"/>
  <circle cx="74" cy="184" r="15" fill="var(--panel)" stroke="var(--accent)" stroke-width="1.5"/>
  <text x="74" y="189" text-anchor="middle" font-size="13" font-weight="700" fill="var(--accent-ink)">2</text>
  <text x="100" y="180" font-size="13" font-weight="700" fill="var(--ink)">Subagent least-privilege</text>
  <text x="100" y="199" font-size="10.5" fill="var(--muted)">strip DELEGATE_BLOCKED_TOOLS · only just-enough tools</text>
  <text x="340" y="216" text-anchor="middle" font-size="13" fill="var(--faint)">▾</text>
  <rect x="40" y="218" width="600" height="52" rx="9" fill="var(--accent-soft)" stroke="var(--accent)"/>
  <circle cx="74" cy="244" r="15" fill="var(--panel)" stroke="var(--accent)" stroke-width="1.5"/>
  <text x="74" y="249" text-anchor="middle" font-size="13" font-weight="700" fill="var(--accent-ink)">3</text>
  <text x="100" y="240" font-size="13" font-weight="700" fill="var(--ink)">Injection isolation</text>
  <text x="100" y="259" font-size="10.5" fill="var(--muted)">gateway's two guards · control split from the data stream (ch.18)</text>
  <text x="340" y="276" text-anchor="middle" font-size="13" fill="var(--faint)">▾</text>
  <rect x="40" y="278" width="600" height="52" rx="9" fill="var(--accent-soft)" stroke="var(--accent)"/>
  <circle cx="74" cy="304" r="15" fill="var(--panel)" stroke="var(--accent)" stroke-width="1.5"/>
  <text x="74" y="309" text-anchor="middle" font-size="13" font-weight="700" fill="var(--accent-ink)">4</text>
  <text x="100" y="300" font-size="13" font-weight="700" fill="var(--ink)">Supply-chain lock</text>
  <text x="100" y="319" font-size="10.5" fill="var(--muted)">every dep pinned (== / commit SHA) · trust nothing that auto-changes</text>
  <text x="340" y="338" text-anchor="middle" font-size="13" fill="var(--accent)">▾</text>
  <rect x="158" y="342" width="364" height="44" rx="9" fill="var(--panel-2)" stroke="var(--line)"/>
  <text x="340" y="369" text-anchor="middle" font-size="12" font-weight="700" fill="var(--ink)">🔒 System · real privileges (terminal / files / browser / messaging)</text>
  <text x="340" y="408" text-anchor="middle" font-size="12" font-weight="700" fill="var(--muted)">one layer falls, the next catches · never rely on the model's own judgment</text>
</svg>
<div class="fig-cap"><b>Defense in depth, four layers</b>: to reach Hermes's real privileges, a malicious input must pass — <b>in series</b> — four <b>independent</b> gates: ① dangerous-command approval (<span class="mono">detect_dangerous_command</span> regex blacklist; HARDLINE blocks even <span class="mono">/yolo</span>) → ② subagent least-privilege (strip <span class="mono">DELEGATE_BLOCKED_TOOLS</span>) → ③ injection isolation (gateway's two guards, ch.18) → ④ supply-chain lock (all deps pinned). Each layer gates with <b>deterministic code</b>: <b>one layer falls, the next catches; never rely on the model's own judgment</b>.</div>
</div>
...
<div class="figure">
<svg viewBox="0 0 680 300" role="img" aria-label="HARDLINE versus DANGEROUS: two tiers of dangerous commands graded by irreversibility">
  <text x="340" y="22" text-anchor="middle" font-size="13.5" font-weight="700" fill="var(--ink)">Two tiers of dangerous commands · locked by irreversibility</text>
  <rect x="28" y="38" width="300" height="200" rx="10" fill="var(--red-soft)" stroke="var(--red)"/>
  <rect x="28" y="38" width="300" height="34" rx="10" fill="var(--red)"/>
  <text x="178" y="60" text-anchor="middle" font-size="12.5" font-weight="700" fill="var(--panel)">HARDLINE · 12 · red lines</text>
  <g font-size="11" font-family="monospace" fill="var(--ink)">
    <text x="44" y="98">rm -rf /  ·  /home /etc /usr</text>
    <text x="44" y="122">mkfs (format filesystem)</text>
    <text x="44" y="146">dd of=/dev/sd* (raw disk)</text>
    <text x="44" y="170">:(){ :|:&amp;};:  (fork bomb)</text>
  </g>
  <rect x="44" y="186" width="246" height="26" rx="13" fill="var(--red)"/>
  <text x="167" y="204" text-anchor="middle" font-size="11" font-weight="700" fill="var(--panel)">⛔ blocks even /yolo · zero tolerance</text>
  <text x="178" y="230" text-anchor="middle" font-size="10.5" fill="var(--red)">irreversible · no legit reason · unbounded cost</text>
  <rect x="352" y="38" width="300" height="200" rx="10" fill="var(--amber-soft)" stroke="var(--amber)"/>
  <rect x="352" y="38" width="300" height="34" rx="10" fill="var(--amber)"/>
  <text x="502" y="60" text-anchor="middle" font-size="12.5" font-weight="700" fill="var(--panel)">DANGEROUS · 61 · yellow lines</text>
  <g font-size="11.5" fill="var(--ink)">
    <text x="368" y="98">a match requires user approval</text>
    <text x="368" y="122">/yolo can clear it once</text>
    <text x="368" y="146">smart-approval auto-clears low-risk</text>
    <text x="368" y="170">this layer only loosens, never red lines</text>
  </g>
  <rect x="368" y="186" width="246" height="26" rx="13" fill="var(--amber)"/>
  <text x="491" y="204" text-anchor="middle" font-size="11" font-weight="700" fill="var(--panel)">⚠ /yolo can clear · convenience door</text>
  <text x="502" y="230" text-anchor="middle" font-size="10.5" fill="var(--amber)">reversible · controlled setting · expert user</text>
  <text x="70" y="270" font-size="11" font-weight="700" fill="var(--red)">◀ irreversible (welded)</text>
  <text x="340" y="270" text-anchor="middle" font-size="11" fill="var(--muted)">graded by irreversibility</text>
...
<svg viewBox="0 0 680 524" role="img" aria-label="Real data flow of one obfuscated command r-backslash-m -rf /home/alice/* being restored to rm -rf ~/* and then blocked by the HARDLINE regex: (1) the attack input hides escaped backslashes and empty quotes to dodge the regex; (2) _normalize_command_for_detection runs multi-step normalization (strip_ansi removes ANSI escapes, strip null bytes, NFKC normalize, re.sub strips backslashes turning r-backslash-m into rm, strip empty quotes, _rewrite_resolved_user_home folds /home/alice/ into ~/) restoring rm -rf ~/*; (3) it matches the HARDLINE regex recursive delete of home directory; (4) _hardline_block_result hard-blocks with BLOCKED hardline, refused even with --yolo /yolo approvals.mode=off; (5) contrast: if we instead asked the model whether it is dangerous, it might answer this is a safe cleanup script and allow it, whereas the regex matches deterministically and wording tricks are useless.">
  ...
  <text x="58" y="194" font-size="9" fill="var(--muted)">3  unicodedata.normalize('NFKC')</text>
  <text x="58" y="212" font-size="9" fill="var(--purple)">4  re.sub(r'\\([^\n])', r'\1')</text>
  <text x="232" y="212" font-size="9" fill="var(--muted)">strip backslashes r\m→rm · approval.py:575</text>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>Dangerous-command approval</strong>: <span class="mono">detect_dangerous_command</span> uses a regex blacklist (12 HARDLINE + 61 DANGEROUS); HARDLINE red lines block even <span class="mono">/yolo</span>.</li>
...
<div class="figure">
<svg viewBox="0 0 680 436" role="img" aria-label="一个真实决策：技能为何注入为 append-only user 消息——build_skill_invocation_message 同一函数同时命中三条设计线。① 决策点：用户敲 /my-skill，是重建 system prompt 前缀破缓存，还是 append 一条 user 消息？② 真实实现 agent/skill_commands.py:517，def build_skill_invocation_message 返回 Optional[str]，docstring Build the user message content for a skill slash command，返回值作为 user 消息 append、不进 system prompt。③ 命中线一缓存：源码注释 does NOT invalidate the skills system-prompt cache、preserves prefix caching、skills called by name。④ 命中线三窄腰：技能不占核心 schema，_HERMES_CORE_TOOLS 仅 skills_list、skill_view、skill_manage 三入口。⑤ 命中线二自我进化：学到的外置成文件、只追加、不改写。⑥ 反事实：若改塞进 system prompt，每装一个技能前缀就逐字节变，从该点起全对话 prompt 缓存作废、全价重算。">
  <text x="20" y="22" font-size="13.5" font-weight="700" fill="var(--ink)">一个决策：技能为何注入为 append-only user 消息</text>
  <text x="20" y="42" font-size="10" fill="var(--muted)">build_skill_invocation_message 同一函数同时满足缓存、进化、窄腰三条设计线</text>
  ...
  <text x="54" y="136" font-size="10.5" font-weight="700" fill="var(--purple)">② 真实实现 · agent/skill_commands.py:517</text>
  <text x="58" y="156" font-size="9" fill="var(--ink)">def build_skill_invocation_message(...) -&gt; Optional[str]:</text>
  ...
  <text x="50" y="328" font-size="9" fill="var(--muted)">skills called by name</text>
  ...
  <text x="460" y="352" font-size="9" fill="var(--faint)">skill_commands.py:517</text>
...
<div class="card key">
  <div class="tag">📌 本课要点</div>
  <ul>
    <li><strong>一个敌人</strong>：全书所有工程都在对抗同一句话——「一个<strong>无状态、会幻觉、分不清指令与数据</strong>的概率内核，要在真实世界里长期自主、可靠地运行」；一切确定性都外置到代码与文件。</li>
    <li><strong>三条主线</strong>：<strong>缓存神圣</strong>（前缀逐字节稳定）、<strong>自我进化</strong>（学到的外置成文件、append-only）、<strong>窄腰</strong>（核心薄、能力在边缘）——三线交汇：窄腰让核心稳、稳核心让缓存活、活缓存让成本低。</li>
    <li><strong>缓存神圣线</strong>：长对话每轮复用逐字节稳定的前缀，改一个字符就作废、成本翻倍；压缩是<strong>唯一例外</strong>，控制命令旁路、后台开独立会话，都为不碰那段已焐热的前缀。</li>
    <li><strong>自我进化线</strong>：技能 append 为 user 消息、记忆沉淀进 <span class="mono">MEMORY.md</span>、Curator 后台养护——进化发生在<strong>文件系统与会话边界</strong>，绝不改流式中的 prefix。</li>
    <li><strong>窄腰线</strong>：核心工具越少、每轮发的 schema 越短、缓存前缀越小越稳；能力全长在<strong>插件 / 技能 / MCP 边缘</strong>，边缘出问题也不碰核心。</li>
    <li><strong>安全横切</strong>：绝不让会被注入的概率模型<strong>当安全裁判</strong>，信任边界钉死在确定性代码上（第 24 章）。</li>
  </ul>
</div>
...
<div class="figure">
<svg viewBox="0 0 680 436" role="img" aria-label="One real decision: why a skill is injected as an append-only user message, where build_skill_invocation_message satisfies all three design lines at once. (1) Decision point: when a user types /my-skill, rebuild the system-prompt prefix and break the cache, or append one user message? (2) Real implementation agent/skill_commands.py:517, def build_skill_invocation_message returns Optional[str], docstring Build the user message content for a skill slash command, the return value is appended as a user message and never enters the system prompt. (3) Hits line 1 the sacred cache: source comment does NOT invalidate the skills system-prompt cache, preserves prefix caching, skills called by name. (4) Hits line 3 the narrow waist: skills add no core schema, _HERMES_CORE_TOOLS exposes only skills_list, skill_view, skill_manage. (5) Hits line 2 self-evolution: learnings externalized to files, append-only, never rewritten. (6) Counterfactual: if instead stuffed into the system prompt, every skill install changes the prefix byte-for-byte and voids the whole conversation cache, recomputed at full price.">
  <text x="20" y="22" font-size="13.5" font-weight="700" fill="var(--ink)">One decision: why a skill is an append-only user message</text>
  <text x="20" y="42" font-size="10" fill="var(--muted)">build_skill_invocation_message — one function satisfies the cache, evolution, and narrow-waist lines at once</text>
  <rect x="40" y="54" width="600" height="46" rx="9" fill="var(--blue-soft)" stroke="var(--blue)"/>
  ...
  <text x="340" y="112" text-anchor="middle" font-size="13" fill="var(--faint)">▾</text>
  <rect x="40" y="116" width="600" height="80" rx="9" fill="var(--purple-soft)" stroke="var(--purple)" stroke-width="2"/>
  <text x="54" y="136" font-size="10.5" font-weight="700" fill="var(--purple)">② Real implementation · agent/skill_commands.py:517</text>
  <text x="58" y="156" font-size="9" fill="var(--ink)">def build_skill_invocation_message(...) -&gt; Optional[str]:</text>
  <text x="58" y="172" font-size="9" fill="var(--muted)">&quot;&quot;&quot;Build the user message content for a skill slash command</text>
...
<div class="card key">
  <div class="tag">📌 Key points</div>
  <ul>
    <li><strong>One enemy</strong>: every piece of engineering in the book fights the same sentence — "a <strong>stateless, hallucination-prone, instruction-vs-data-blind</strong> probabilistic core must run autonomously, reliably, long-term in the real world"; all determinism is pushed out into code and files.</li>
    <li><strong>Three main lines</strong>: <strong>sacred cache</strong> (the prefix is byte-stable), <strong>self-evolution</strong> (learnings externalized to files, append-only), <strong>narrow waist</strong> (thin core, capability at the edges) — they converge: the narrow waist keeps the core stable, the stable core keeps the cache alive, the live cache keeps cost low.</li>
    <li><strong>Sacred-cache line</strong>: a long conversation reuses a byte-stable prefix every turn; change one character and it voids, doubling cost; compression is the <strong>only exception</strong>, control commands bypass, background tasks open isolated sessions — all to avoid touching that already-warm prefix.</li>
    <li><strong>Self-evolution line</strong>: skills append as a user message, memory distills into <span class="mono">MEMORY.md</span>, the Curator tends skills in the background — evolution happens on the <strong>filesystem and at session boundaries</strong>, never mutating the streaming prefix.</li>
    <li><strong>Narrow-waist line</strong>: fewer core tools means a shorter schema sent each turn and a smaller, stabler cached prefix; all capability grows at the <strong>plugin / skill / MCP edge</strong>, and an edge failure never touches the core.</li>
    <li><strong>Security cross-cut</strong>: never let an injectable probabilistic model <strong>be the safety judge</strong>; the trust boundary is nailed to deterministic code (ch.24).</li>
  </ul>
</div>
