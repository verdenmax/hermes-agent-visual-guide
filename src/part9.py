...
<div class="card macro">
  ...
  <p><strong>③ 落盘的原子性，挂在一个毫不起眼的 rename 上。</strong>一个写到一半的配置或会话文件就是损坏；正确的做法是先写临时文件，再用 <span class="mono">os.replace()</span> 把它<strong>原子换上</strong>，让读者要么看到旧文件、要么看到完整的新文件，<strong>永远不会读到撕裂的中间态</strong>。</p>
...
<div class="card macro">
  ...
  <p><strong>③ On-disk atomicity hangs on one unremarkable rename.</strong> A half-written config or session file is corruption; the fix is to write a temp file and <span class="mono">os.replace()</span> it into place, so a reader only ever sees either the old file or the whole new one — <strong>never a torn middle</strong>.</p>
